	...
	public LinkedList load(String filename) {
		...
		if(filename.endsWith("xml")) {
			...
			currenttype = "bif";
                        ...
                        System.out.println("load IDEAL file ...");
                        System.out.println("load variables ...");
...
for(int i=0;i<nodeNames.size();i++) {
  ...
  return nodesInfo;
        ...
        private void loadDependenciesIDEAL(String sbuf) {
          ...
          blockName = sbuf.substring(sbuf.indexOf("(")+1, sbuf.indexOf(" ")+1).trim();

          parentInfo = sbuf.substring(sbuf.indexOf("PREDECESSORS-* ")+15, sbuf.indexOf(":SUCCESSORS")).trim();

          if(!parentInfo.equals("NIL")){
            while(parentInfo.indexOf("NODE-REF :NAME")>0){
              String tmp = parentInfo.substring(parentInfo.indexOf("NODE-REF :NAME ")+15, parentInfo.indexOf(")"));
              parentList.addElement(tmp.trim());
              parentInfo = parentInfo.substring(parentInfo.indexOf(")")+1);
            }
          }
          probabilitiesList = sbuf.substring(sbuf.indexOf(":ARRAY #(")+9, sbuf.indexOf(":MARKOV-BLANKET")).trim();
          probabilitiesList = probabilitiesList.substring(0, probabilitiesList.length()-1);
...
System.out.println("blockName = " + blockName);
          String states = sbuf.substring(sbuf.indexOf("Labels==>")+10, sbuf.indexOf("#s")).trim();
...
System.out.println("states = " + states);
          String coordinates = sbuf.substring(sbuf.indexOf("ERGO-SCREEN-COORDINATE")+23, sbuf.length());
          xcoordinate = Integer.parseInt(coordinates.substring(0, coordinates.indexOf(" ")));
          ycoordinate = Integer.parseInt(coordinates.substring(coordinates.indexOf(" "), coordinates.indexOf(")")).trim());
...
*/
	...
	private void loadNodeXMLBIF(String sbuf, boolean currentVer) {
		...
		endProperty = sbuf.indexOf("</PROPERTY>",endCoordinates);
                ...
                xcoordinate = new Double(sbuf.substring(startCoordinates+1,endCoordinates)).doubleValue();
                ycoordinate = new Double(sbuf.substring(endCoordinates+2,endProperty-1)).doubleValue();
...
//		ycoordinate = new Integer(sbuf.substring(endCoordinates+2,endProperty-1)).intValue();
	...
	private String trimexcess(String sbuf) {
         ...
         */
        private void addItemToBayNet(String blockName, double xcoordinate, double ycoordinate, Vector stateList) {
                System.out.println("stateList ******* " + stateList.toString());
                ChanceBlock cb;
		Ellipse2D.Double e2d;
		Item item;
